...
import { createQueryMapFeaturesTool, createRunMapCommandTool } from './runtime.js';
...
const commandResult = (result: AiStyleToolResult<MapCommandReceipt>): MapCommandReceipt => {
  assert.equal(result.success, true, result.success ? '' : `${result.error.code}: ${result.error.message}`);
  if (!result.success) throw new Error('unreachable');
  return result.data;
};
const queryResult = (result: AiStyleToolResult<FeatureQueryProjection>): FeatureQueryProjection => {
  assert.equal(result.success, true, result.success ? '' : `${result.error.code}: ${result.error.message}`);
  if (!result.success) throw new Error('unreachable');
  return result.data;
};
...
describe('runtime AI tools', () => {
  ...
  it('forwards SDK abort signals only to URL image commands', async () => {
    ...
    ) => Promise<AiStyleToolResult<MapCommandReceipt>>;

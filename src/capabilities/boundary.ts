...
export const MAX_CAPABILITY_OUTPUT_BYTES = 1_048_576;
export const MAX_CAPABILITY_OUTPUT_ITEMS = 100;
export const MAX_CAPABILITY_OUTPUT_WARNINGS = 20;
export const MAX_CAPABILITY_MESSAGE_BYTES = 4_096;
...
const normalizedMessage = (message: string): string => truncateUtf8(message, MAX_CAPABILITY_MESSAGE_BYTES);
export const issuePointer = (path: PropertyKey[]): string | undefined => path.length === 0
  ? undefined
  : `/${path.map((part) => String(part).replace(/~/g, '~0').replace(/\//g, '~1')).join('/')}`;

type MutableData = { warnings: StyleWarning[]; truncated: boolean };

const finalizeSuccess = <T extends MutableData>(message: string, data: T, limit: number, omitted: boolean): CapabilityResult<T> => {
...
const admitWarnings = <T extends MutableData>(
  ...
  let omitted = warnings.length > MAX_CAPABILITY_OUTPUT_WARNINGS;
  for (const warning of warnings.slice(0, MAX_CAPABILITY_OUTPUT_WARNINGS)) {
...
const boundedLimit = (value: number | undefined): number => Math.min(value ?? MAX_CAPABILITY_OUTPUT_BYTES, MAX_CAPABILITY_OUTPUT_BYTES);
...
export const boundInspectionProjection = (input: {
  ...
  if ('items' in input.projection) {
    ...
    if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) throw new RangeError('AI result mandatory envelope exceeds output limit.');
    const warningsOmitted = admitNestedWarnings(
      output, projection, input.warnings, MAX_CAPABILITY_OUTPUT_BYTES,
    );
    let omitted = input.truncated === true || warningsOmitted;
    for (const item of input.projection.items.slice(0, MAX_CAPABILITY_OUTPUT_ITEMS)) {
      projection.items.push(item); projection.returned = projection.items.length;
      if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) { projection.items.pop(); projection.returned = projection.items.length; omitted = true; break; }
    }
  ...
  if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) { delete projection.value; projection.returned = 0; omitted = true; }
  projection.truncated = omitted;
  if (!omitted) projection.warnings.pop();
  if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) throw new RangeError('AI result mandatory envelope exceeds output limit.');
  return output;
...
export const boundStyleMutationReceipt = (input: {
...
}): CapabilityResult<StyleMutationReceipt> => {
  ...
  const { output, valid } = reserve(input.message, data, MAX_CAPABILITY_OUTPUT_BYTES);
  if (!valid) throw new RangeError('AI result mandatory envelope exceeds output limit.');
  let omitted = admitWarnings(output, input.warnings, MAX_CAPABILITY_OUTPUT_BYTES);
  const admit = <T>(source: readonly T[], target: T[]): void => {
    for (const item of source.slice(0, MAX_CAPABILITY_OUTPUT_ITEMS - data.changedLayers.length - data.changedSources.length)) {
      target.push(item);
      if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) { target.pop(); omitted = true; break; }
    }
  ...
  return finalizeSuccess(input.message, data, MAX_CAPABILITY_OUTPUT_BYTES, omitted);
};
...
export const boundMapCommandReceipt = (input:
...
): CapabilityResult<MapCommandReceipt> => {
  ...
  const { output, valid } = reserve(input.message, data, MAX_CAPABILITY_OUTPUT_BYTES);
  if (!valid) throw new RangeError('AI result mandatory envelope exceeds output limit.');
  let omitted = input.truncated === true || admitWarnings(output, input.warnings, MAX_CAPABILITY_OUTPUT_BYTES);
  if (input.kind === 'list') {
    ...
    for (const item of input.result.items.slice(0, MAX_CAPABILITY_OUTPUT_ITEMS)) {
      result.items.push(item); result.returned = result.items.length;
      if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) { result.items.pop(); result.returned = result.items.length; omitted = true; break; }
    }
    ...
    if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) { delete data.result; omitted = true; }
  }
  return finalizeSuccess(input.message, data, MAX_CAPABILITY_OUTPUT_BYTES, omitted);
};
...
export const boundFeatureQueryProjection = (input: {
...
}): CapabilityResult<FeatureQueryProjection> => {
  ...
  for (const feature of input.features.slice(0, MAX_CAPABILITY_OUTPUT_ITEMS)) {
...
export const invalidInputFailure = (error: z.ZodError): CapabilityResult<never> => {
  const issue = error.issues[0];
  return toFailure(createStyleToolError(
    'INVALID_INPUT',
    issue?.message ?? 'Tool input is invalid.',
    issue === undefined ? undefined : issuePointer(issue.path),
  ));
};

export const toFailure = (error: StyleToolError): CapabilityResult<never> => {
  const message = normalizedMessage(error.message);
  const path = error.path === undefined ? undefined : truncateUtf8(error.path, MAX_CAPABILITY_MESSAGE_BYTES);
  ...
  if (error.details !== undefined) {
    ...
    if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) snapshot.details = { outputTruncated: true };
    if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) delete snapshot.details;
  }
  if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) delete snapshot.path;
  if (jsonUtf8ByteLength(output) > MAX_CAPABILITY_OUTPUT_BYTES) throw new RangeError('AI failure envelope exceeds output limit.');
  return output;
};

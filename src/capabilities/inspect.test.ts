...
import type { CapabilityResult, InspectionProjection, InspectStyleInput, MapToolContext } from './contracts.js';
import { executeInspectStyle } from './inspect.js';
import { MapStyleAuthority } from './map-authority.js';

const createInspectStyleTool = (options: {
  getMap: () => MapLibreMap | null;
  getContext?: () => MapToolContext;
}) => ({
  execute: (input: unknown) => executeInspectStyle(() => {
...
const value = (result: CapabilityResult<InspectionProjection>): Record<string, unknown> => {
...
const items = (result: CapabilityResult<InspectionProjection>): Record<string, unknown>[] => {
  assert.equal(result.success, true);

...
import type { MapLibreStyleTools } from '../ai/tools.js';

const assertResultNarrowing = (result: CapabilityResult<InspectionProjection>): void => {
  if (result.success) {
...
const assertToolSignatures = (tools: MapLibreStyleTools): void => {
  const inspect: (input: InspectStyleInput) => Promise<CapabilityResult<InspectionProjection>> = tools.inspectStyle.execute;
  const transaction: (input: ApplyStyleTransactionInput) => Promise<CapabilityResult<StyleMutationReceipt>> = tools.applyStyleTransaction.execute;
  const document: (input: ApplyStyleDocumentInput) => Promise<CapabilityResult<StyleMutationReceipt>> = tools.applyStyleDocument.execute;
  const command: (input: RunMapCommandInput) => Promise<CapabilityResult<MapCommandReceipt>> = tools.runMapCommand.execute;
  const query: (input: QueryMapFeaturesInput) => Promise<CapabilityResult<FeatureQueryProjection>> = tools.queryMapFeatures.execute;
...
describe('bounded AI result boundary', () => {
  it('bounds the complete feature envelope in source order', () => {
    ...
    assert.ok(jsonUtf8ByteLength(bounded) <= MAX_CAPABILITY_OUTPUT_BYTES);
  ...
  it('omits an oversized first list item atomically', () => {
    const bounded = boundMapCommandReceipt({
      ...
      result: { items: ['x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES), 'later'] }, warnings: [],
    });
    assert.equal(bounded.success, true);
  ...
  it('rejects an item that fits one marker but not both reserved markers', () => {
    const bounded = boundMapCommandReceipt({
      message: 'ok', action: 'listImages', kind: 'list', applied: true,
      result: { items: ['x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES - 380), 'later'] }, warnings: [],
    });
    assert.ok(jsonUtf8ByteLength(bounded) <= MAX_CAPABILITY_OUTPUT_BYTES);
    assert.equal(bounded.success, true);
    if (bounded.success && bounded.data.result && typeof bounded.data.result === 'object' && 'items' in bounded.data.result) {
  ...
  it('caps messages and omits oversized atomic values', () => {
    const bounded = boundInspectionProjection({
      ...
      action: 'getRoot',
      projection: { value: { payload: 'x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES) } },
      warnings: [],
    });
    assert.equal(bounded.success, true);
    if (bounded.success) {
  ...
  it('stops nested inspection collections at the first omitted item', () => {
    const bounded = boundInspectionProjection({
      ...
      action: 'listLayers',
      projection: { items: [{ id: 1 }, { payload: 'x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES) }, { id: 3 }], total: 3 },
      warnings: [],
    });
    assert.equal(bounded.success, true);
  ...
  it('allocates command lists before marking their truncation', () => {
    const bounded = boundMapCommandReceipt({
      ...
      kind: 'list',
      applied: true,
      result: { items: ['first', 'x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES), 'later'] },
      warnings: [],
    });
    assert.equal(bounded.success, true);
    if (bounded.success && bounded.data.result && typeof bounded.data.result === 'object' && 'items' in bounded.data.result) {
  ...
  it('lowers but never raises the feature byte cap', () => {
    const high = boundFeatureQueryProjection({ message: 'ok', target: 'source', features: oversizedFeatures, warnings: [], maxSerializedBytes: MAX_CAPABILITY_OUTPUT_BYTES * 2 });
    ...
    assert.ok(jsonUtf8ByteLength(high) <= MAX_CAPABILITY_OUTPUT_BYTES);
  ...
  it('bounds oversized authenticated error details', () => {
    const failure = toFailure(createStyleToolError('INTERNAL', 'error', undefined, { payload: 'x'.repeat(MAX_CAPABILITY_OUTPUT_BYTES) }));
    assert.equal(failure.success, false);
    assert.ok(jsonUtf8ByteLength(failure) <= MAX_CAPABILITY_OUTPUT_BYTES);

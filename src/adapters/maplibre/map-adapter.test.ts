...
import {
  ...
  createLegacyMapLifecycleFacade,
...
function minimalMapFacade(map: MinimalStyleMap): MapLibreMap {
  return createLegacyMapLifecycleFacade(map as unknown as MapLibreMap);
}

test('legacy lifecycle facade preserves real Maps and verifies minimal synchronous installs', async () => {
  const real = new FakeMap(rawStyle()).asMap();
  assert.strictEqual(createLegacyMapLifecycleFacade(real), real);

  let current = rawStyle();
  let getStyleCalls = 0;
  const setStyleCalls: SetStyleCall[] = [];
  const minimal: MinimalStyleMap = {
    getStyle() {
      getStyleCalls += 1;
      return structuredClone(current);
    },
    setStyle(style, options) {
      setStyleCalls.push({ style, options });
      assert.notEqual(typeof style, 'string');
      current = structuredClone(style as StyleSpecification);
      return minimal;
    },
  };
  const facade = minimalMapFacade(minimal);
  let loads = 0;
  let errors = 0;
  facade.on('style.load', () => { loads += 1; });
  facade.on('error', () => { errors += 1; });

  facade.setStyle(rawStyle('#fff'), { diff: false });
  await flushUntil(() => loads === 1);

  assert.equal(errors, 0);
  assert.equal(facade.isStyleLoaded(), true);
  assert.deepEqual(setStyleCalls.map((call) => call.options), [{ diff: false }]);
  assert.equal(specificationColor(facade.getStyle()), '#fff');
  assert.equal(getStyleCalls >= 2, true, 'verification and public reads both delegate');
});

test('legacy lifecycle facade always delegates getStyle and never caches its candidate', async () => {
  let current = rawStyle();
  let getStyleCalls = 0;
  const minimal: MinimalStyleMap = {
    getStyle() {
  ...
  let loads = 0;
  facade.on('style.load', () => { loads += 1; });
  facade.setStyle(rawStyle('#fff'));
  await flushUntil(() => loads === 1);

  current = rawStyle('#123');
  assert.equal(specificationColor(facade.getStyle()), '#123');
  current = rawStyle('#456');
  assert.equal(specificationColor(facade.getStyle()), '#456');
  assert.equal(getStyleCalls, 3, 'one verification read plus two fresh public reads');
});

test('legacy lifecycle facade rejects silent and unresolved installs without faking current style', async () => {
  for (const mode of ['silent', 'unresolved'] as const) {
    const baseline = rawStyle();
    let getStyleCalls = 0;
    const minimal: MinimalStyleMap = {
      getStyle() {
        getStyleCalls += 1;
      ...
      setStyle() {
        if (mode === 'unresolved') return new Promise<never>(() => {});
        return minimal;
      },
    };
    const facade = minimalMapFacade(minimal);
    ...
    facade.setStyle(rawStyle('#fff'));
    await flushUntil(() => errors === 1);

    assert.equal(loads, 0, mode);
    assert.equal(facade.isStyleLoaded(), false, mode);
    assert.equal(specificationColor(facade.getStyle()), '#000', mode);
    assert.equal(getStyleCalls, 2, mode + ': verification plus fresh public read');
  }
});

test('legacy lifecycle facade failure settles transaction within the supplied deadline', async () => {
  const baseline = rawStyle();
  const minimal: MinimalStyleMap = {
    getStyle: () => structuredClone(baseline),
    setStyle: () => minimal,
  };
  ...
  const result = await Promise.race([
    applyTransactionToMap(minimalMapFacade(minimal), colorTransaction('#fff'), {
      deadline: { expiresAt: Date.now() + deadlineMs },
    }),
    new Promise<typeof timedOut>((resolve) => {
      timer = setTimeout(() => { resolve(timedOut); }, deadlineMs + 250);
    }),
  ]);
  if (timer !== undefined) clearTimeout(timer);

  assert.notEqual(result, timedOut);
  if (result === timedOut) assert.fail('legacy lifecycle transaction did not settle');
  assert.equal(result.ok, false);
  assert.equal(result.styleAuthority, 'current');
  assert.equal(result.rolledBack, true);
  assert.equal(styleColor(result.style), '#000');
});

...
`maplibre-style-tools` exists so that an AI agent can read and change a MapLibre style without endangering it. That is the purpose of the library. Agents are the primary caller here, and everything in the package, from the five operations down to the shape of an error message, is designed around what an AI can and cannot be trusted to do.

## What goes wrong without it

Consider an application with a MapLibre map where an assistant should handle requests such as "make the water bluer" or "hide the POI labels". The direct approach is to give the model access to the map object or a raw `setStyle` call. In practice this fails in predictable ways:

- The model invents a layer ID. Nothing checks it, so the request goes through and the style is silently wrong.
- It sends a filter as a JSON-encoded string instead of an expression array. A parse error then appears far from the actual mistake.
- Two of its three operations succeed and one fails. The style is now half-changed, and it is unclear which half.
...
- While the model is thinking, the user changes something in another tab. The edit lands on a style that no longer exists.

These are not unusual cases; they are the expected failure modes of pointing a probabilistic caller at an imperative rendering API. This package therefore does not expose the map. It exposes the workflow a careful operator would follow: inspect first, apply deliberate changes, then verify what changed.
...
Next: [Architecture](../architecture/) explains how the package is organized, or continue to [Installation](../../getting-started/installation/).

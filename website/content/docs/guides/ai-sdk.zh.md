...
`/ai` 把能力注册表变成 AI SDK 6 的工具集，作用在你应用持有的地图上。一次工厂调用：模型拿到五个描述清晰的工具，地图始终收在你控制的回调之后。
...
输入是原生 JSON 值 —— 对象和数组就传对象和数组，不要编码成字符串。执行时校验照常进行。

## 地图可用性跟随应用生命周期

`getMap` 在工具执行时求值。页面加载中或地图销毁后返回 `null`，调用方就会得到正常的 `MAP_NOT_READY` 失败 —— 没有过期的地图引用，也不会崩溃。

## 结果只需要一种处理模式

成功带 `data`；失败带包创建的 `StyleToolError`。先看 `result.success`，再各取所需。检查类操作返回紧凑投影 —— 真需要完整样式文档时，通过 [`/core`](../core/) 或 Map 实例读取。

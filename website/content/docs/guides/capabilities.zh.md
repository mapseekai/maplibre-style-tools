...
`inputSchema` 是执行器校验的严格边界；`modelInputSchema` 是模型工具定义对外公布的形状。模型 schema 负责引导生成，执行时依然会重新校验，所以模型无法靠编造字段绕过边界。

## OpenAI 与 Anthropic 的 schema 投影

`createOpenAiFunctionTools()` 和 `createAnthropicTools()` 把注册表投影成 OpenAI function-tool 与 Anthropic 工具定义，适合直接集成 LLM API。它们只定义工具：不挂地图，也不执行任何东西。把定义和权威源配对，在你的处理器里调用对应的注册表执行器。

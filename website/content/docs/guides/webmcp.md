...
`supported: false` means this browser does not expose `document.modelContext`. WebMCP is still a Community Group draft, so check the [draft specification](https://webmachinelearning.github.io/webmcp/) and [Web Platform Test results](https://wpt.fyi/results/webmcp) rather than a browser-version list.

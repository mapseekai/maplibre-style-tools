...
The browser-oriented `/maplibre`, `/bridge`, and `/webmcp` entry points belong in a browser or MapLibre host. The Node-oriented `/ai` and `/mcp` entry points may use Node types; the CLI also runs on Node.js. The AI SDK interface needs a Node-capable AI SDK 6 host and AI SDK `^6.0.141`.

Choose the interface whose authority matches your task: an in-process map for AI SDK tools, or a local Style file for the CLI.
